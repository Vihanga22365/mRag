...
def generate_text_summaries(texts, tables, summarize_texts=False):
    ...
    if time_hist_color == 'gpt-4-turbo':
        model = ChatOpenAI(temperature=0, model= "gpt-4-turbo", openai_api_key = openai.api_key, max_tokens=1024)

    elif time_hist_color == 'gemini-1.5-pro-latest':
        model = ChatGoogleGenerativeAI(temperature=0, model="gemini-1.5-pro-latest", max_output_tokens=1024)
        
    elif time_hist_color == 'Claude 3.5 Sonnet':
        model = ChatAnthropic(temperature=0, model="claude-3-5-sonnet-20240620", api_key=claude_api_key, max_tokens=1024)
    else:
...
def img_prompt_func2(data_dict):
    ...
    text_message = {
        ...
        "text": (
            ...
            "Use this information to provide answers related to the user question. \n"
            "Final answer should be easily readable and structured. \n"
            "Final answer should be summarized to 10 words. \n"
            f"User-provided question: {data_dict['question']}\n\n"
            "Text and / or tables:\n"
            f"{formatted_texts}"
        ),
